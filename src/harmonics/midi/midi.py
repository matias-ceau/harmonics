...
from ..harmonics import Harmonics, timer
from ..theory.elements import Pitch
...
class MidiUtils:
    ...
    def midi_cc(cls, data1):
        return Harmonics.MIDI_CC.get(data1)

    @classmethod
    def midi_program(cls, channel, data1):
        if channel == 9:
            return Harmonics.DRUM_PROG.get(data1)
        else:
            return Harmonics.MIDI_GM1.get(data1)

    @classmethod
    def midi_drum(cls, midi):
        return Harmonics.DRUM_KEYMAP.get(midi)

    @classmethod
    def midi_to_note(cls, midi):
        return [a for a, b in Harmonics.NDICT.items() if midi % 12 == b][0]
...
"""
    ...
    def _extract_midi(self):
        ...
        for n, track in enumerate(self.MIDI.tracks):
            ...
            msgtranslate = Harmonics.MIDIMESSAGE
            df = pd.DataFrame({
        ...
        vd = np.vectorize(Harmonics.midi_drum)
        ...
        vcc = np.vectorize(Harmonics.midi_cc)
        ...
        vpr = np.vectorize(Harmonics.midi_program)
