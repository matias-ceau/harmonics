...
def all_notes(sequence: Sequence, *args) -> int:
    ...
    if sequence.shape[0] >= 10:
        if np.unique(sequence % 12).shape[0] == 7:
            return 1
        else:
            return 0
    else:
...
(first tests the presence of such leaps,
 ...
 (half of sum of differences is still under p4)'''
    if np.abs(np.diff(sequence).max()) <= 5:
        ...
        if np.sum([np.sign(np.prod(i)) == -1 for i in test])\
...
def mutation(sequence: Sequence,
    ...
    for _ in range(num):
        ...
        sequence[index] = sequence[index] if np.random.random(
        ) > probability else note
    return sequence
    ...
    for i in range(generation_limit):
        ...
        if popD.score.max() >= fitness_limit:
            ...
            with open(os.path.join(
                    _folder,
                    'data',
                    'counter_point_arrays',
                    f'{key_center}_{sequence_length}_{1000*end}.npy'),
                    'wb') as f:
                np.save(f, cantus)
            break
